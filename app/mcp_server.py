...
def create_mcp_server(settings: Settings) -> FastMCP[None]:
    ...
    server: FastMCP[None] = FastMCP(
        ...
        instructions=(
            ...
            "Set purchase_channel on every expense, never leave it at its unknown default for an "
            "ordinary receipt: in_store for a normal purchase made in person at a physical location — "
            "this is the right answer whenever nothing on the receipt says otherwise, not something to "
            "actively detect; delivery when a third-party delivery platform is involved (Uber Eats, "
            "Instacart, DoorDash, SkipTheDishes, or similar — look for their branding, a delivery or "
            "service fee line, or a driver tip); online for a direct order placed with the merchant "
            "itself and shipped or picked up rather than delivered by a platform (an e-commerce order "
            "confirmation, a 'ship to' address, an online order number format); subscription for a "
            "recurring membership or service charge rather than a one-time purchase (a streaming "
            "service, a membership renewal). Leave it unknown only when the receipt genuinely gives no "
            "signal either way — that should be rare, not the common case. "
